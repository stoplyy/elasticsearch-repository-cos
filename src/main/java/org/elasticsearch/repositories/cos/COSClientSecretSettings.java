...
public class COSClientSecretSettings {
    ...
    private static final String COS_CLIENT_PREFIX_KEY = "ts.cos.client.";
    /** cos account name */
    public static final AffixSetting<SecureString> ACCOUNT_SETTING =
        Setting.affixKeySetting(COS_CLIENT_PREFIX_KEY, "account", key -> SecureSetting.secureString(key, null));

    /** cos SecretId */
    public static final AffixSetting<SecureString> KEY_SETTING =
        Setting.affixKeySetting(COS_CLIENT_PREFIX_KEY, "secret_id", key -> SecureSetting.secureString(key, null));

    /** cos SecretKey */
    public static final AffixSetting<SecureString> SECRET_SETTING = Setting.affixKeySetting(COS_CLIENT_PREFIX_KEY, "secret_key",
    ...
    public static Map<String, COSClientSecretSettings> load(Settings settings) {
        ...
        if (false == storageSettings.containsKey("cos_default") && false == storageSettings.isEmpty()) {
            // in case no setting named "cos_default" has been set, let's define our "cos_default"
    ...
    private static COSClientSecretSettings getClientSettings(Settings settings, String account) {
        try (SecureString accountSecret = getConfigValue(settings, account, ACCOUNT_SETTING);
    ...
    private static <T> T getConfigValue(Settings settings, String account,
